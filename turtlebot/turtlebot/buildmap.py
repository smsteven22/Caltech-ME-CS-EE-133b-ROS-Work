...
LFREE     = -0.063         # Set the log odds ratio of detecting freespace
LOCCUPIED = 0.042          # Set the log odds ratio of detecting occupancy
...
class CustomNode(Node):
    ...
    def sendMap(self):
        ...
        # Convert the log-odds-ratio into a probability.
        
        """
        Problem 3
        """
        probability = np.zeros((HEIGHT, WIDTH))
        for x in range(len(self.logoddsratio)):
            for y in range(len(self.logoddsratio[x])):
                probability[x,y] = np.exp(self.logoddsratio[x,y]) / (1 + np.exp(self.logoddsratio[x,y]))
    ...
    def laserCB(self, msg):
        ...
        #############################################################
        # PROCESS THE LASER SCAN TO UPDATE THE LOG ODDS RATIO!
        #############################################################
        """
        Problem 4
        """
        ...
        for i in range(len(ranges)):
            ...
            if rmin < r and r < rmax:
                xr = r * np.cos(theta + thetac)
                yr = r * np.sin(theta + thetac)
                u = int(((xc + xr) - ORIGIN_ X) / RESOLUTION)
                v = int (((yc + yr) - ORIGIN_Y) / RESOLUTION)
                self. set(u, v, 1)
        """
        ...
        for i in range(len(ranges)):
            ...
            if rmin < r and r < rmax:
                xr = r * np.cos(theta + thetac)
                yr = r * np.sin(theta + thetac)
                xrmin = rmin * np.cos(theta + thetac)
                yrmin = rmin * np.sin(theta + thetac)

                us = int(((xc + xrmin) - ORIGIN_X) / RESOLUTION)
                vs = int(((yc + yrmin) - ORIGIN_Y) / RESOLUTION)
                ue = int(((xc + xr) - ORIGIN_X) / RESOLUTION)
                ve = int(((yc + yr) - ORIGIN_Y) / RESOLUTION)

                for (u, v) in self.bresenham((us, vs), (ue, ve)):
                    self.adjust(u, v, LFREE)

                self.adjust(ue, ve, LOCCUPIED)
            elif r == rmax:
                xr = r * np.cos(theta + thetac)
                yr = r * np.sin(theta + thetac)
                xrmin = rmin * np.cos(theta + thetac)
